...
function initialRandomNumArray(){
    for (var i = 0; i < 16; ++i){
...
//生成不重复的随机数
function getNotRepeatedRandomNum(){
...
function isAdjoin(a, b){
    //确定当前点击块和空白块的行列信息
    var a_col = parseInt(a.className.charAt(18));
    var a_row = parseInt(a.className.charAt(24));
    var b_col = parseInt(b.className.charAt(18));
    var b_row = parseInt(b.className.charAt(24));
...
function click_picture_block(){
    //游戏未开始，没有行为
    if (myPuzzle.isStart == 0) document.getElementById("display_window").innerText = "Click the button to start!";
    else {
        //定位空白快
